...
class DialogueRequest(BaseModel):
    ...
    current_trip: TripPlan = Field(..., description="当前行程信息")
    conversation_history: List[Message] = Field(default=[], description="对话历史")
...
class DialogueResponse(BaseModel):
    """对话响应"""
    success: bool = Field(..., description="是否成功")
    recognized_intents: List[str] = Field(default=[], description="识别到的意图列表")
    agent_responses: List[AgentResponse] = Field(default=[], description="各Agent的响应")
    general_response: Optional[str] = Field(default=None, description="普通对话响应")
    modified_trip: Optional[TripPlan] = Field(default=None, description="修改后的行程（如果有）")
    requires_trip_modification: bool = Field(default=False, description="是否需要用户确认行程修改")
