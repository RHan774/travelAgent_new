<template>
  <div class="result-container">
    ...
    <div v-if="tripPlan" class="content-wrapper">
      ...
      <!-- 主内容区 -->
      <div class="main-content" :class="{ 'content-updating': isUpdating }">
        <!-- 顶部信息区:左侧概览+预算,右侧地图 -->
    ...
    <!-- 右侧对话面板 -->
    <div class="chat-panel" :class="{ 'chat-panel-open': chatPanelOpen }">
      <div class="chat-header">
        <span class="chat-title">💬 旅行助手</span>
        <a-button type="text" size="small" @click="toggleChatPanel">
          {{ chatPanelOpen ? '−' : '+' }}
        </a-button>
      </div>
      
      <div v-if="chatPanelOpen" class="chat-content">
        <div class="chat-messages" ref="chatMessagesRef">
          <div v-if="chatMessages.length === 0" class="chat-welcome">
            <div class="welcome-icon">🤖</div>
            <div class="welcome-text">您好！我是您的旅行助手</div>
            <div class="welcome-hint">您可以问我关于景点、天气、酒店的问题，或者告诉我您想如何修改行程</div>
          </div>
          ...
          <div v-for="(msg, index) in chatMessages" :key="index" class="chat-message" :class="msg.role">
            ...
            <div class="message-content">
              <div v-if="msg.role === 'assistant' && msg.agentResponses" class="agent-responses">
                <div v-for="(agentResp, ai) in msg.agentResponses" :key="ai" class="agent-response">
              ...
              <!-- 行程修改确认 -->
              <div v-if="msg.role === 'assistant' && msg.pendingTripModification" class="trip-modification-prompt">
                <a-divider>行程修改建议</a-divider>
                <div class="modification-buttons">
                  <a-button type="primary" @click="confirmTripModification(msg)">
                    ✅ 应用修改
                  </a-button>
                  <a-button @click="cancelTripModification(msg)">
                    ❌ 取消
                  </a-button>
                </div>
              </div>
...
const pendingModifiedTrip = ref<TripPlan | null>(null)
...
const getAgentLabel = (agentType: string): string => {
  const labels: Record<string, string> = {
    search_attraction: '[景点搜索结果]',
    weather_query: '[天气查询结果]',
    hotel_recommend: '[酒店推荐结果]',
    modify_trip: '[行程修改建议]'
  }
...
const handleChatInput = async () => {
  if (!chatInput.value.trim() || !tripPlan.value) return
...
const processDialogueResponse = async (response: DialogueResponse) => {
  ...
  if (response.modified_trip) {
    pendingModifiedTrip.value = response.modified_trip
  }

  chatMessages.value.push(assistantMessage)
}

const confirmTripModification = async (msg: any) => {
  if (!pendingModifiedTrip.value) return

  isUpdating.value = true

  // 更新行程
  tripPlan.value = pendingModifiedTrip.value
  sessionStorage.setItem('tripPlan', JSON.stringify(tripPlan.value))

  // 清除待修改状态
  msg.pendingTripModification = false
  pendingModifiedTrip.value = null
...
const cancelTripModification = (msg: any) => {
  msg.pendingTripModification = false
  pendingModifiedTrip.value = null
  message.info('已取消行程修改')
}
</script>
...
.result-container {
  min-height: 100vh;
  background: linear-gradient(135deg, #f0f4ff 0%, #e8eefc 50%, #fdf2f8 100%);
  padding: 48px 24px;
  position: relative;
}
...
.weather-wind {
  margin-top: 12px;
  padding-top: 12px;
  border-top: 2px solid rgba(0, 77, 64, 0.15);
  text-align: center;
  color: #004d40;
  font-size: 15px;
  font-weight: 600;
...
.agent-label.search_attraction {
  background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%);
}

.agent-label.weather_query {
  background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%);
}

.agent-label.hotel_recommend {
  background: linear-gradient(135deg, #fa709a 0%, #fee140 100%);
}

.agent-label.modify_trip {
  background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
}
...
.chat-input-area .ant-input-textarea {
  flex: 1;
}
